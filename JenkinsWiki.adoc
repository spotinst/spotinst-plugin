...
* Pending Timeout Behavior Change:
* Previous Behavior: It was possible to set a value above 15 minutes for the pending timeout, but the maximum timeout was capped at 15 minutes. If the field was left empty, the timeout was set to 10 minutes.
* New Behavior: Any value set for the pending timeout will now be respected, removing the previous 15-minute limitation. The default remains 10 minutes. If you explicitly set a value, it will be honored without the previous cap.
